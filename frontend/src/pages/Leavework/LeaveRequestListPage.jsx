    ...
    return (
        <div>
            <div className="d-flex justify-content-between align-items-center mb-3">
                {/* <<<<<<< ตรงนี้คือจุดที่แก้ไข: เปลี่ยน style={{ fontSize: '2rem' }} เป็น style={{ fontSize: '1.8rem' }} >>>>>>> */}
                <h4 className="fw-bold text-dark" style={{ fontSize: '1.8rem' }}>รายการคำขอลา (รอดำเนินการ)</h4> 
                <Link to="/leave-requests/history" className="btn btn-outline-secondary" style={{ fontSize: '1rem' }}>
                    <FontAwesomeIcon icon={faHistory} className="me-2" /> ประวัติการลา
                </Link>
            </div>
            <p className="text-muted" style={{ fontSize: '0.95rem' }}>หน้าหลัก / <span className="text-dark">รายการคำขอลา</span></p>
                        ...
                        <table className="table table-hover table-bordered text-center align-middle">
                            <thead className="table-light">
                                <tr>
                                    ...
                                    <th style={{ fontSize: '1.05rem', color: '#333' }}>ดำเนินการ</th>
                                </tr>
                            </thead>
                            <tbody>
                                        ...
                                        <td style={{minWidth: '180px'}}>
                                            {leave.leavework_status === 'pending' ? (
                                                <div className="d-flex justify-content-center gap-2">
                                                    <button onClick={() => handleUpdateStatus(leave.leavework_id, 'approved')} className="btn btn-success btn-sm" style={{ fontSize: '0.95rem' }}>อนุมัติ</button>
                                                    <button onClick={() => handleUpdateStatus(leave.leavework_id, 'rejected')} className="btn btn-danger btn-sm" style={{ fontSize: '0.95rem' }}>ไม่อนุมัติ</button>
                                                </div>
                                            ) : (
                                                <span className="text-muted" style={{ fontSize: '0.98rem' }}>-</span>
                                            )}
                                        </td>
                                    </tr>
                                )) : (
                                        ...
                                        <td colSpan="6" className="text-center text-muted p-4">
                                            <div className="d-flex flex-column justify-content-center align-items-center">
                                                ...
                                                <span className="mb-0 text-muted" style={{ fontSize: '1.05rem' }}>
                                                    {filters.search || filters.leaveworktype_id ? 'ไม่พบข้อมูลคำขอลาตามเงื่อนไข' : 'ไม่พบข้อมูลคำขอลาที่รอดำเนินการตามเงื่อนไข'}
                                                </span>
                                            </div>
                                        </td>
                                    </tr>
