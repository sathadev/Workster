...
function PublicJobPostingDetailPage() {
  ...
  const formatDate = (dateString) => {
    ...
    return date.toLocaleDateString('th-TH', { year: '2-digit', month: 'short', day: '2-digit' });
  };
                ...
                <div className="job-detail-content">
                  <div className="d-flex justify-content-between align-items-start">
                    <div>
                      ...
                      <p className="text-muted mb-1">{job.job_location_text || '-'}</p>
                      <p className="fs-5">
                        {job.salary_min?.toLocaleString()} - {job.salary_max?.toLocaleString()} บาท
                      </p>
                    </div>
                  ...
                  <div className="job-detail-section mb-5">
                    ...
                    <p className="mb-1"><span className="fw-bold">โทรศัพท์ :</span> {job.contact_phone || '-'}</p>
                    <p className="mb-1"><span className="fw-bold">อีเมล :</span> {job.contact_email || '-'}</p>
                  </div>

                  <div className="text-end">
                    <button className="btn btn-primary px-4" disabled={job.job_status !== 'active'}>
                      {job.job_status === 'active' ? 'ยืนยันสมัครงาน' : 'ปิดรับสมัคร'}
                    </button>
                  </div>
                </div>
              </div>
