...
import { Card, Button, Alert, Spinner, Container, Row, Col, ListGroup } from 'react-bootstrap';
...
function PublicJobPostingDetailPage() {
    ...
    if (loading) return <div className="text-center mt-5 text-muted"><Spinner animation="border" /> กำลังโหลดรายละเอียดประกาศงาน...</div>;
    ...
    if (!jobPosting) return <div className="alert alert-warning mt-5 text-center" style={{ fontSize: '0.95rem' }}>ไม่พบข้อมูลประกาศนี้</div>;

    const isApplicationOpen = jobPosting.job_status === 'active';
    const qualifications = jobPosting.qualifications_text ? jobPosting.qualifications_text.split("\n").filter(Boolean) : [];
    const benefits = jobPosting.benefits_text ? jobPosting.benefits_text.split("\n").filter(Boolean) : [];

    return (
        <Container className="py-4">
            <div className="d-flex justify-content-between align-items-center mb-3">
                <h4 className="fw-bold text-dark" style={{ fontSize: '1.8rem' }}>รายละเอียดประกาศงาน</h4>
                <Button variant="outline-secondary" onClick={() => window.history.back()} style={{ fontSize: '0.9rem' }}>
                ...
                <Card.Body className="p-4 p-md-5">
                    <div className="job-detail-content">
                        <div className="d-flex justify-content-between align-items-start">
                            <div>
                                <h3 className="fw-bold" style={{ fontSize: '2rem' }}>{jobPosting.job_title}</h3>
                        ...
                        {jobPosting.job_description && (
                            <div className="mb-4">
                                <h5 className="fw-bold text-dark mb-3" style={{ fontSize: '1.3rem' }}>รายละเอียดงาน</h5>
                                <p className="text-secondary" style={{ whiteSpace: 'pre-wrap' }}>{jobPosting.job_description}</p>
                            </div>
                        )}
                        <hr className="my-4" />
                        ...
                        {qualifications.length > 0 && (
                            <div className="mb-4">
                                <h5 className="fw-bold text-dark mb-3" style={{ fontSize: '1.3rem' }}>คุณสมบัติผู้สมัคร</h5>
                                <ul className="list-unstyled ps-4">
                                    {qualifications.map((q, i) => <li key={i} className="text-secondary"><FontAwesomeIcon icon={faClipboardList} className="me-2 text-info" />{q}</li>)}
                                </ul>
                            </div>
                        )}
                        ...
                        {benefits.length > 0 && (
                            <div className="mb-4">
                                ...
                                <ul className="list-unstyled ps-4">
                                    {benefits.map((b, i) => <li key={i} className="text-secondary"><FontAwesomeIcon icon={faClipboardList} className="me-2 text-info" />{b}</li>)}
                                </ul>
                            </div>
                        )}
                        ...
                        {jobPosting.contact_person_name && (
                            <div className="mb-4">
                                <h5 className="fw-bold text-dark mb-3" style={{ fontSize: '1.3rem' }}>ติดต่อ</h5>
                                <ul className="list-unstyled">
                                    <li className="text-secondary"><strong>ผู้ติดต่อ:</strong> {jobPosting.contact_person_name || "-"}</li>
                                    <li className="text-secondary"><strong>โทรศัพท์:</strong> {jobPosting.contact_phone || "-"}</li>
                                    <li className="text-secondary"><strong>อีเมล:</strong> {jobPosting.contact_email || "-"}</li>
                                </ul>
                            </div>
                        )}
