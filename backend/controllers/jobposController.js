    ...
    exports.getPositionById = async (req, res) => {
        try {
            ...
            const [position, employeesInPos] = await Promise.all([
                ...
                Employee.getByJobposId(id, req.companyId) // EmployeeModel ไม่ต้องสนใจisSuperAdmin
            ]);
    ...
    exports.createPosition = async (req, res) => {
        try {
            ...
            // ลบ Logic ที่เกี่ยวข้องกับ Super Adminในการสร้าง Global jobpos ออก
            // ตอนนี้ Admin/HR ปกติสร้างjobpos ที่มี company_id เป็นของตัวเอง
            ...
            const newPosition = await Jobpos.create(jobpos_name.trim(), targetCompanyIdForJobpos); // <-- ลบ isSuperAdminออก
    ...
    exports.updatePosition = async (req, res) => {
        try {
            ...
            const updatedPosition = await Jobpos.update(id, jobpos_name.trim(), req.companyId); // <-- ลบ isSuperAdminออก
            if (!updatedPosition) {
    ...
    // --- NEW: Public Endpoint สำหรับ Job Positions ---
    /**
     * @desc ดึงข้อมูลตำแหน่งงานทั้งหมด (Global) สำหรับ Public View
     * @route GET /api/v1/public/positions
     * @access Public
     */
    exports.getPublicPositions = async (req, res) => {
        try {
            // ดึงเฉพาะตำแหน่งงานที่มี company_id เป็น NULL (Global positions)
            const positions = await Jobpos.getAll(null); // ส่ง null เพื่อให้ Model ดึง Global positions
            res.status(200).json(positions.filter(pos => pos.jobpos_id !== 0)); // กรอง Super Admin ออก
        } catch (err) {
